
- *建议使用vscode打开此工程*
- *跳转的链接都是 `[链接名字](文件路径#L14)` 表示对应文件的第14行,如果是md格式的则建议使用#标题*
- *按照时间线顺序来记录created shenshuxin 20230328*
- *持续更新中，同时发布到[csdn](https://blog.csdn.net/weixin_48835367?type=blog)*
---
<br>

# 面试IT
- [自我介绍](./detail_files/%E9%9D%A2%E8%AF%95/%E8%87%AA%E6%88%91%E4%BB%8B%E7%BB%8D.md)

# linux命令
- [常用命令](./detail_files/linux/linux-command.md#L1)
- [linux下查询生产日志（简单实用）](https://blog.csdn.net/weixin_48835367/article/details/126533287?spm=1001.2014.3001.5502)
- [Linux（centos7.9）系统下命令行连接蓝牙设备](https://blog.csdn.net/weixin_48835367/article/details/124697443?spm=1001.2014.3001.5502)

# python基础
- [基础知识](./detail_files/python/python-learn.md#基础知识)
- [自定义包setup](./detail_files/python/python-learn.md#自定义包setup)
- [应用项目2022](./detail_files/python/python-learn.md#应用项目2022)
- [pip3使用安装python3的依赖](./detail_files/python/python-learn.md#pip3使用安装python3的依赖)

# java基础
- [java基础语法](./detail_files/javaSE/java-learn.md#java基础语法)
- [多线程](./detail_files/javaSE/java-learn.md#多线程)
- [网络io nio](./detail_files/javaSE/java-learn.md#网络io)
- [长时长视频java存储及vue播放解决方法](https://blog.csdn.net/weixin_48835367/article/details/128722845?spm=1001.2014.3001.5502)
- [java实现【国密SM4】加密解密-CBC模式](https://blog.csdn.net/weixin_48835367/article/details/123914276?spm=1001.2014.3001.5502)
- [java生产者消费者同步模式](./detail_files/javaSE/java-learn.md#java生产者消费者同步模式)
- [函数式接口function](./detail_files/javaSE/java-learn.md#函数式接口function)
- [optional处理null值](./detail_files/javaSE/java-learn.md#optional处理null值)
- [单例模式常用实现](./detail_files/javaSE/java-learn.md#单例模式常用实现)

# oracle
- [sql语法](./detail_files/oracle/oracle-learn.md#sql语法)
- [sql优化](./detail_files/oracle/oracle-learn.md#sql优化)
- [mysql too many connections](./detail_files/oracle/oracle-learn.md#mysql-too-many-connections-解决方法)
- [postgres10版本匿名函数二维数组使用](./detail_files/oracle/oracle-learn.md#postgres10版本匿名函数二维数组使用)
- [oracle报错对象不存在](https://blog.csdn.net/weixin_48835367/article/details/123403913?spm=1001.2014.3001.5502)

# ETL
- [informatica](./detail_files/ETL/etl-learn.md#informatica)
- [kettle](./detail_files/ETL/etl-learn.md#kettle)

# spring
## spring
- [核心组件](./detail_files/spring/spring/spring-learn.md#核心组件)
- [ioc原理](./detail_files/spring/spring/spring-learn.md#ioc原理)
- [aop原理](./detail_files/spring/spring/spring-learn.md#aop原理)
- [logback时区设置东八区-生产环境配置](./detail_files/spring/spring/spring-learn.md#logback时区设置东八区-生产环境配置)

## springMVC
- [springmvc执行流程](./detail_files/spring/springMVC/springmvc-learn.md#执行流程)

## mybatis
- [mybatis的一级二级缓存](./detail_files/spring/mybatis/mybatis-learn.md#mybatis的一级二级缓存)
- [自定义mybatis插件](./detail_files/spring/mybatis/mybatis-learn.md#自定义mybatis插件)
- [mybatis-plus使用](./detail_files/spring/mybatis/mybatis-learn.md#mybatis-plus使用)
- [mybatis参数查询多个参数，不同类型，参数方法查询](./detail_files/spring/mybatis/mybatis-learn.md#mybatis参数查询多个参数不同类型参数方法查询)

## ssm框架
- [ssm](./detail_files/spring/SSM%E7%BB%8F%E5%85%B8%E6%A1%86%E6%9E%B6/SSM.md)
- [ssm项目本地开发启动步骤](./detail_files/spring/SSM%E7%BB%8F%E5%85%B8%E6%A1%86%E6%9E%B6/SSM.md#ssm项目本地开发启动步骤)
- [ssm整合：这是第三次自己整合了](https://blog.csdn.net/weixin_48835367/article/details/117533594?spm=1001.2014.3001.5502)


# maven
- [profiles机制](./detail_files/maven/maven-learn.md#profiles)
- [自定义maven插件plugin](./detail_files/maven/maven-learn.md#自定义maven插件plugin)
- [jar包管理仓库nexus](./detail_files/maven/nexus-learn.md)
- [关于maven下载依赖失败问题](./detail_files/maven/nexus-learn.md#关于maven下载依赖失败问题)
- [maven下载依赖指定仓库地址](./detail_files/maven/maven-learn.md#maven下载依赖指定仓库地址)


# 脚本
## bat脚本
- [win10设置每日bing壁纸](./detail_files/%E8%84%9A%E6%9C%AC/win10%E6%AF%8F%E6%97%A5%E6%9B%B4%E6%8D%A2bing%E5%A3%81%E7%BA%B8/bing_backgrounds_picture_everyday.bat)

# angular
- [ng基础](./detail_files/angular/angular-learn.md#ng基础)
- [html转义符](./detail_files/angular/angular-learn.md#html转义符)
- [js的同步执行](https://blog.csdn.net/weixin_48835367/article/details/126033476?spm=1001.2014.3001.5502)


# vue
- [vue基础](./detail_files/vue/vue-learn.md#vue基础)
- [部署vue3开发环境](https://blog.csdn.net/weixin_48835367/article/details/124173547?spm=1001.2014.3001.5502)
- [项目welcome-ssx](./detail_files/vue/vue-learn.md#应用2022年项目welcome-ssx)
- [vue3集成elementUI](https://blog.csdn.net/weixin_48835367/article/details/124176604?spm=1001.2014.3001.5502)
- [vue3嵌套路由](https://blog.csdn.net/weixin_48835367/article/details/124173994?spm=1001.2014.3001.5502)

# spring boot
- [springboot-bean生命周期](./detail_files/spring-boot/springboot-learn.md#springboot-bean生命周期)
- [springboot自动配置的原理](./detail_files/spring-boot/springboot-learn.md#springboot自动配置的原理)
- [starter机制](./detail_files/spring-boot/springboot-learn.md#starter机制)
- [springboot启动流程](./detail_files/spring-boot/springboot-learn.md#springboot启动流程)
- [网站下载文件到本地](https://blog.csdn.net/weixin_48835367/article/details/124990146?spm=1001.2014.3001.5502)
- [springboot集成logback配置文件模板](https://blog.csdn.net/weixin_48835367/article/details/122936047?spm=1001.2014.3001.5502)


# spring cloud
- [项目welcome-ssx](./detail_files/spring-cloud/springcloud-learn.md#应用2022年项目welcome-ssx)
- [服务注册发现](./detail_files/spring-cloud/springcloud-learn.md#服务注册发现)
- [远程配置中心](./detail_files/spring-cloud/springcloud-learn.md#远程配置中心)
- [rpc](./detail_files/spring-cloud/springcloud-learn.md#rpc)
- [熔断](./detail_files/spring-cloud/springcloud-learn.md#熔断)
- [负载均衡](./detail_files/spring-cloud/springcloud-learn.md#负载均衡)
- [k8s部署elk+filebeat+springCloud集成elk+filebeat+kafka+zipkin](https://blog.csdn.net/weixin_48835367/article/details/128751984?spm=1001.2014.3001.5502)
- [spring cloud gateway整合security实现统一权鉴](https://blog.csdn.net/weixin_48835367/article/details/127716081?spm=1001.2014.3001.5502)


# https
- [请求流程](./detail_files/https/https-learn.md#请求流程)
- [tcp三次握手](./detail_files/https/tcp-learn.md#tcp三次握手)
- [tcp四次挥手](./detail_files/https/tcp-learn.md#tcp四次挥手)
- [nginx部署](./detail_files/https/https-learn.md#nginx部署)
- [nginx完全指南](./detail_files/https/https-learn.md#nginx完全指南)
- [nginx功能](./detail_files/https/https-learn.md#nginx功能)
- [配置项目外网访问（公网IP+DDNS）](https://blog.csdn.net/weixin_48835367/article/details/127714985?spm=1001.2014.3001.5502)
- [nginx转发](./detail_files/https/https-learn.md#nginx转发)


# redis
- [redis数据类型及操作命令](./detail_files/redis/redis-learn.md#redis数据类型及操作命令)
- [持久化方式](./detail_files/redis/redis-learn.md#持久化方式)

# dubbo
- [dubbo](./detail_files/dubbo/dubbo-learn.md)

# gRPC
- [gRPC](./detail_files/gRPC/grpc-learn.md)

# spring security
- [spring security](./detail_files/spring-security/springsecurity-learn.md)

# seata
- [at模式](./detail_files/seata/seata-learn.md#at模式)
- [tcc模式](./detail_files/seata/seata-learn.md#tcc模式)

# 分布式锁
- [zookeeper锁](./detail_files/%E5%88%86%E5%B8%83%E5%BC%8F%E9%94%81/cloudlock-learn.md#zookeeper锁)
- [redis锁](./detail_files/%E5%88%86%E5%B8%83%E5%BC%8F%E9%94%81/cloudlock-learn.md#redis锁)

# vscode
- [vscode](./detail_files/vscode/vscode-learn.md)

# google插件
- [插件下载地址](./detail_files/Google%E6%8F%92%E4%BB%B6/google-plugin.md#插件google网址)
- [高阶bing搜索引擎](./detail_files/Google%E6%8F%92%E4%BB%B6/google-plugin.md#高阶bing搜索引擎)

# nodeJS
- [nodejs基础](./detail_files/nodeJS/nodejs-learn.md#nodejs基础)

# github Pages
- [github静态页面](shenshuxin01.github.io/mainLogin/index.html)
- [github静态页面源代码](https://github.com/shenshuxin01/shenshuxin01.github.io)

# VCS(git\svn)
- [SVN快速上手使用（适用于新入职同事）](https://blog.csdn.net/weixin_48835367/article/details/124285409?spm=1001.2014.3001.5502)
- [java集成svn实现自动更新提交代码](./detail_files/VCS/vcs-learn.md#java集成svn实现自动更新提交代码)
- [git操作提交-撤销提交-拣选-合并rebase](https://blog.csdn.net/weixin_48835367/article/details/126648919?spm=1001.2014.3001.5502)
- [git冲突解决-详细版本](https://blog.csdn.net/weixin_48835367/article/details/124211809?spm=1001.2014.3001.5502)


# kubernetes
- [k8s部署应用nginx](https://blog.csdn.net/weixin_48835367/article/details/122403725?spm=1001.2014.3001.5502)
- [k8s部署sleuth-zipkin](https://blog.csdn.net/weixin_48835367/article/details/128751964?spm=1001.2014.3001.5502)
- [k8s部署kafka+zookeeper](https://blog.csdn.net/weixin_48835367/article/details/128747588?spm=1001.2014.3001.5502)
- [k8s修改deployment文件的内容（例如修改pod副本数量）](https://blog.csdn.net/weixin_48835367/article/details/124636010?spm=1001.2014.3001.5502)
- [k8s部署redis单机版](https://blog.csdn.net/weixin_48835367/article/details/123690146?spm=1001.2014.3001.5502)
- [k8s部署elasticsearch、kibana（包含数据挂载VOLUME）](https://blog.csdn.net/weixin_48835367/article/details/123219790?spm=1001.2014.3001.5502)
- [k8s部署jenkins](https://blog.csdn.net/weixin_48835367/article/details/123169999?spm=1001.2014.3001.5502)
- [k8s部署nexus3](https://blog.csdn.net/weixin_48835367/article/details/122913229?spm=1001.2014.3001.5502)
- [k8s部署mysql](https://blog.csdn.net/weixin_48835367/article/details/122790697?spm=1001.2014.3001.5502)


# docker
- [docker清理镜像配合grep](./detail_files/docker/docker-learn.md#docker清理镜像配合grep)
- [docker运行docker本地私有仓库服务](./detail_files/docker/docker-learn.md#docker运行docker本地私有仓库服务)
- [使用Dockerfile制作jre环境基础镜像](https://blog.csdn.net/weixin_48835367/article/details/122993320?spm=1001.2014.3001.5502)
- [使用Dockerfile部署docker生成容器-模板](https://blog.csdn.net/weixin_48835367/article/details/122954863?spm=1001.2014.3001.5502)

# esp32
- [esp32电路图](./detail_files/ESP32/esb32-learn.md#esp32电路图)
- [esp32开发板学习](https://blog.csdn.net/weixin_48835367/article/details/125363909?spm=1001.2014.3001.5502)
- [自己配置外接屏幕和驱动，最终通过hdmi连接](https://blog.csdn.net/weixin_48835367/article/details/122660564?spm=1001.2014.3001.5502)

# selenium
- [如何获取动态生成的html信息](https://blog.csdn.net/weixin_48835367/article/details/125101167?spm=1001.2014.3001.5502)
- [selenium官网](./detail_files/selenium/selenium-learn.md#selenium官网)

# hbase
- [java操作hbase使用hbase-client2依赖](https://blog.csdn.net/weixin_48835367/article/details/123007902?spm=1001.2014.3001.5502)

# 正则regex
- [正则regex](./detail_files/%E6%AD%A3%E5%88%99regex/regex-learn.md)

# YApi
- [YApi接口文档,可以把swagger接口转换mock假接口调用](https://yapi.uihtm.com/index/)

# leetCode算法数据结构
- [算法](./detail_files/LeetCode%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84%E5%92%8C%E7%AE%97%E6%B3%95/lc-algorithms-learn.md)
- [数据结构](./detail_files/LeetCode%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84%E5%92%8C%E7%AE%97%E6%B3%95/lc-data-structure-learn.md)

# home bridge
- [home bridge](./detail_files/home-bridge/homebridge-learn.md)

# home assisant
- [home assisant](./detail_files/home-assisant/homeassisant-learn.md)

# untitled
- [untitled基础](./detail_files/md#基础)

# untitled
- [untitled基础](./detail_files/md#基础)

# untitled
- [untitled基础](./detail_files/md#基础)

# untitled
- [untitled基础](./detail_files/md#基础)

# untitled
- [untitled基础](./detail_files/md#基础)

# untitled
- [untitled基础](./detail_files/md#基础)

# untitled
- [untitled基础](./detail_files/md#基础)

# untitled
- [untitled基础](./detail_files/md#基础)

# untitled
- [untitled基础](./detail_files/md#基础)

# untitled
- [untitled基础](./detail_files/md#基础)















