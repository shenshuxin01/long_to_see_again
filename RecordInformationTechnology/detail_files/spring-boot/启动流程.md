...
## 首先调用BeanDefinitionRegistryPostProcessor的实现类
beanFactory.getBeanNamesForType(BeanDefinitionRegistryPostProcessor.class)获取列表，然后遍历：
如果其中的bean实现PriorityOrdered类，调用postProcessBeanDefinitionRegistry();
如果其中的bean实现Ordered类，调用postProcessBeanDefinitionRegistry();
最后执行列表中剩余的，调用postProcessBeanDefinitionRegistry(),注意这一步是循环调用直到所有的都执行完毕，因为过程中可能会注册新的BeanDefinitionRegistryPostProcessor类

## 然后调用BeanFactoryPostProcessor实现类
依然是按照PriorityOrdered类、Ordered类、普通类顺序执行调用postProcessBeanFactory()方法
...
## 在springboot启动时，通过ConfigurationClassPostProcessor类执行@ComponentScan等扫描
`public class ConfigurationClassPostProcessor implements BeanDefinitionRegistryPostProcessor,PriorityOrdered等`
创建AnnotationConfigServletWebServerApplicationContext构造方法中注册了ConfigurationClassPostProcessor到beanFactory。
ConfigurationClassPostProcessor类实现了postProcessBeanDefinitionRegistry()方法，其中最重要的方法是`public void parse(项目启动类Application)`
this.componentScanParser.parse(componentScan, sourceClass.getMetadata().getClassName());
把开发中定义的bean都加载到beanFactory中
